...
for iteration in range(max_iter):
    # Dynamically adjust fuzziness (as an example)
    dynamic_m = m + 0.5 * (iteration / max_iter)  # increases over time
    ...
    tmp = dist ** (-2 / (dynamic_m - 1))
    u_new = tmp / np.sum(tmp, axis=1, keepdims=True)
...
plt.title('Dynamic Objective Function Fuzzy C-Means (DOFCM)')
