...
# Use all features (or drop 'HeartDisease' if desired)
X = df_encoded.values
...
for iteration in range(max_iter):
    ...
    centers = (u ** m).T @ X_scaled / np.sum(u ** m, axis=0)[:, None]
...
# Simulate Type-2 by adding ±10% uncertainty
uncertainty = 0.1
u_lower = np.clip(u - uncertainty * u, 0, 1)
u_upper = np.clip(u + uncertainty * u, 0, 1)
...
plt.title('Type-2 Fuzzy C-Means (Simulated, Custom Implementation)')
plt.xlabel('PCA Component 1')
plt.ylabel('PCA Component 2')
plt.legend()
plt.show()

# Show membership bounds
membership_df = pd.DataFrame()
for i in range(n_clusters):
    membership_df[f'Cluster_{i}_Lower'] = u_lower[:, i]
    membership_df[f'Cluster_{i}_Upper'] = u_upper[:, i]

print("\nSample membership bounds (first 5 rows):")
